...
5. Open the PR against the `development` branch, not `main`.
...
- Tkinter for UI. Keep widget logic in the relevant module (`overlay.py`, `settings_panel.py`, `setup_wizard.py`).
- Config reads and writes go through `config.py`. Do not access `config.ini` directly from other modules.
- Use `_res()` for any file path that needs to resolve correctly both in development and when compiled with PyInstaller.
